...
function SectionHeader({ label, accent, fontSizeNum }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, borderBottom:`2px solid ${accent}`, paddingBottom:4, marginBottom:4 }}>
      <span style={{ background:accent, color:"#fff", fontSize:fontSizeNum*0.8, fontWeight:700, padding:"1px 6px", borderRadius:3 }}>AT</span>
...
function EditableField({ value, onChange, editMode, multiline, style }) {
  const baseStyle = {
    ...
    resize:"none", transition:"all 0.15s", ...style,
  };
  if (!editMode) return <span style={style}>{value}</span>;
  if (multiline) return <textarea value={value} onChange={e=>onChange(e.target.value)} rows={3} style={{ ...baseStyle, display:"block", lineHeight:1.5 }} />;
  return <input value={value} onChange={e=>onChange(e.target.value)} style={{ ...baseStyle, display:"inline-block" }} />;
}
...
function CVPreview({ data, onDataChange, fontFamily, fontSize, accentColor, editMode }) {
  ...
  const updateExp = (ei,f,v) => { const exp=data.experience.map((e,i)=>i===ei?{...e,[f]:v}:e); onDataChange({...data,experience:exp}); };
  const updateBullet = (ei,bi,v) => { const exp=data.experience.map((e,i)=>{ if(i!==ei)return e; const bullets=e.bullets.map((b,j)=>j===bi?v:b); return{...e,bullets}; }); onDataChange({...data,experience:exp}); };
  const updateSkill = (idx,v) => { const skills=data.skills.map((s,i)=>i===idx?v:s); onDataChange({...data,skills}); };
  ...
  const addBullet = (ei) => { const exp=data.experience.map((e,i)=>i===ei?{...e,bullets:[...e.bullets,"New achievement"]}:e); onDataChange({...data,experience:exp}); };
  const removeBullet = (ei,bi) => { const exp=data.experience.map((e,i)=>{ if(i!==ei)return e; return{...e,bullets:e.bullets.filter((_,j)=>j!==bi)}; }); onDataChange({...data,experience:exp}); };

  const editBorder = editMode ? "1px dashed #f59e0b40" : "none";

  return (
    <div style={{ fontFamily:`'${fontFamily}',Georgia,serif`, fontSize:`${fontSizeNum}px`, color:"#1a1a2e", background:"#fff", padding:"28px 32px", minHeight:"500px", lineHeight:1.5, position:"relative" }}>
      {editMode && (
        <div style={{ position:"absolute", top:8, right:8, background:"#fef3c7", border:"1px solid #f59e0b", borderRadius:6, padding:"3px 10px", fontSize:11, color:"#92400e", fontWeight:600 }}>
          ✏ Editing Mode
        </div>
      )}
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
        <div style={{ flex:1 }}>
          <EditableField value={data.name} onChange={v=>update("name",v)} editMode={editMode} style={{ fontSize:fontSizeNum*1.9, fontWeight:700, color:"#111", letterSpacing:"-0.5px", width:"100%" }} />
          <div style={{ marginTop:2 }}>
            <EditableField value={data.title} onChange={v=>update("title",v)} editMode={editMode} style={{ fontSize:fontSizeNum*1.05, color:"#555" }} />
          </div>
        </div>
      ...
      <div style={{ marginBottom:14, marginTop:6, border:editBorder, borderRadius:4 }}>
        <EditableField value={data.summary} onChange={v=>update("summary",v)} editMode={editMode} multiline style={{ fontSize:fontSizeNum*0.92, color:"#333", width:"100%" }} />
      </div>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:6, margin:"8px 0 14px", border:editBorder, borderRadius:4, padding:editMode?4:0 }}>
        {data.skills.map((s,i)=>(
          <span key={i} style={{ background:`${accent}15`, color:accent, border:`1px solid ${accent}40`, borderRadius:4, padding:"2px 6px", fontSize:fontSizeNum*0.85, display:"flex", alignItems:"center", gap:4 }}>
            {editMode ? (
              <>
                <input value={s} onChange={e=>updateSkill(i,e.target.value)} style={{ background:"transparent", border:"none", outline:"none", color:accent, fontSize:fontSizeNum*0.85, fontFamily:"inherit", width:Math.max(60,s.length*7) }} />
        ...
        {editMode && (
          <button onClick={addSkill} style={{ background:`${accent}20`, color:accent, border:`1px dashed ${accent}`, borderRadius:4, padding:"2px 8px", cursor:"pointer", fontSize:fontSizeNum*0.85, fontFamily:"inherit" }}>+ Add</button>
        )}
      </div>
      ...
      {data.experience.map((exp,i)=>(
        <div key={i} style={{ marginTop:10, border:editBorder, borderRadius:4, padding:editMode?6:0 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <EditableField value={exp.company} onChange={v=>updateExp(i,"company",v)} editMode={editMode} style={{ fontWeight:700, fontSize:fontSizeNum }} />
            <EditableField value={exp.period} onChange={v=>updateExp(i,"period",v)} editMode={editMode} style={{ fontSize:fontSizeNum*0.85, color:"#777", textAlign:"right" }} />
          </div>
          <div style={{ marginBottom:6 }}>
            <EditableField value={exp.role} onChange={v=>updateExp(i,"role",v)} editMode={editMode} style={{ color:accent, fontWeight:600, fontSize:fontSizeNum*0.92 }} />
          </div>
          <ul style={{ margin:0, paddingLeft:18 }}>
            {exp.bullets.map((b,j)=>(
              <li key={j} style={{ fontSize:fontSizeNum*0.88, color:"#444", marginBottom:4, display:"flex", alignItems:"flex-start", gap:4 }}>
                <EditableField value={b} onChange={v=>updateBullet(i,j,v)} editMode={editMode} multiline style={{ fontSize:fontSizeNum*0.88, color:"#444", flex:1 }} />
                {editMode && <span onClick={()=>removeBullet(i,j)} style={{ cursor:"pointer", color:"#ef4444", fontWeight:700, fontSize:14, flexShrink:0, marginTop:2 }}>×</span>}
              </li>
            ))}
          </ul>
          {editMode && (
            <button onClick={()=>addBullet(i)} style={{ marginTop:6, marginLeft:18, background:"transparent", border:`1px dashed ${accent}`, color:accent, borderRadius:4, padding:"2px 10px", cursor:"pointer", fontSize:fontSizeNum*0.82, fontFamily:"inherit" }}>+ Add bullet</button>
          )}
        </div>
      ))}
    </div>
      ...
      <div style={{ flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:8, minHeight:0, paddingRight:2 }}>
        ...
        {loading && (
          <div style={{ display:"flex", justifyContent:"flex-start" }}>
            <div style={{ background:"#f1f5f9", borderRadius:"12px 12px 12px 2px", padding:"8px 14px", fontSize:18, color:"#94a3b8" }}>···</div>
          </div>
        )}
        <div ref={bottomRef} />
      </div>
    ...
    const doc = new Document({
      sections:[{
        ...
        children:[
          ...
          new Paragraph({
            alignment: AlignmentType.CENTER,
            children:[new TextRun({ text:`${cvData.location}  |  ${cvData.email1}  |  ${cvData.email2}`, size:18, color:"777777" })],
            spacing:{ after:200 },
          }),
          new Paragraph({
          ...
          new Paragraph({
            border:{ bottom:{ style:BorderStyle.SINGLE, size:6, color:"1e3a5f" } },
            children:[new TextRun({ text:"KEY SKILLS", bold:true, size:22, color:"1e3a5f" })],
            spacing:{ after:100 },
          }),
          new Paragraph({
            children:[new TextRun({ text:cvData.skills.join("   •   "), size:20, color:"333333" })],
            spacing:{ after:200 },
          }),
          new Paragraph({
            border:{ bottom:{ style:BorderStyle.SINGLE, size:6, color:"1e3a5f" } },
            children:[new TextRun({ text:"PROFESSIONAL EXPERIENCE", bold:true, size:22, color:"1e3a5f" })],
            spacing:{ after:120 },
          }),
          ...cvData.experience.flatMap(exp => [
            ...
            new Paragraph({
              children:[new TextRun({ text:exp.role, size:20, color:accentColor.replace("#",""), italics:true })],
              spacing:{ after:80 },
            }),
            ...exp.bullets.map(b =>
              new Paragraph({
      ...
      <div style={{ display:"flex", flex:1, minHeight:0 }}>
        ...
        <div style={{ flex:1, background:"#f8fafc", overflowY:"auto", padding:"24px 28px" }}>
          ...
          <div ref={cvPreviewRef} style={{
            ...
            <CVPreview data={cvData} onDataChange={setCvData} fontFamily={fontFamily} fontSize={fontSize} accentColor={accentColor} editMode={editMode} />
          </div>
        </div>
